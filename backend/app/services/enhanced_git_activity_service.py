"""
Enhanced Git Activity Service with Caching

Integrates the Git Activity Service with Redis caching for improved performance.
Provides intelligent caching strategies and background cache warming.
"""
...
class GitActivityService:
        ...
        for page in range(1, max_pages + 1):
            ...
            for commit_data in response:
                try:
                    ...
                    logger.warning(f"Failed to process commit {commit_data.get('sha', 'unknown')}: {e}")
                    continue
        ...
        for page in range(1, max_pages + 1):
            ...
            for pr_data in response:
                try:
                    ...
                    logger.warning(f"Failed to process PR {pr_data.get('number', 'unknown')}: {e}")
                    continue
    ...
    async def get_repository_contributors(
        ...
        for contributor_data in response:
            try:
                ...
                logger.warning(f"Failed to process contributor {contributor_data.get('login', 'unknown')}: {e}")
                continue
    ...
    def generate_activity_heatmap(
        ...
        for commit in commits:
            if start_date <= commit.committed_date <= end_date:
                ...
                daily_activity[date_key]["contributors"].add(commit.author_login or commit.author_email)
                daily_activity[date_key]["activity_types"].add(ActivityType.COMMIT.value)
        ...
        for pr in pull_requests:
            if start_date <= pr.created_at <= end_date:
                ...
                daily_activity[date_key]["activity_types"].add(ActivityType.PULL_REQUEST.value)
        ...
        while current_date <= end_date_only:
            ...
            day_data = daily_activity.get(date_key, {
                "commits": [],
                "prs": [],
                "contributors": set(),
                "activity_types": set()
    ...
    def _calculate_velocity_trend(
        self,
        heatmap_data: List[ActivityHeatmapData]
    ) -> List[Dict[str, Any]]:
    ...
    async def get_repository_activity(
        ...
        logger.info(f"Fetching comprehensive activity for {owner}/{repo} ({days_back} days)")
        
        # Fetch data concurrently
        tasks = [
            self.get_repository_commits(access_token, owner, repo, since=start_date, until=end_date),
            self.get_repository_pull_requests(access_token, owner, repo, since=start_date),
            self.get_repository_contributors(access_token, owner, repo)
        ]
        
        commits, pull_requests, contributors = await asyncio.gather(*tasks)
        
        logger.info(f"Fetched {len(commits)} commits, {len(pull_requests)} PRs, {len(contributors)} contributors")
        
        # Generate heatmap
        heatmap_data = self.generate_activity_heatmap(commits, pull_requests, start_date, end_date)
        ...
        avg_pr_size = sum(pr.additions + pr.deletions for pr in merged_prs) / max(len(merged_prs), 1)
        
        top_contributors = sorted(contributors, key=lambda c: c.contributions, reverse=True)[:10]
...
git_activity_service = GitActivityService()
    ...
    async def _make_github_request(
        ...
        Returns:
            Union[Dict[str, Any], List[Dict[str, Any]]]: API response data
        """
        headers = {
            "Authorization": f"token {access_token}",
            "Accept": "application/vnd.github.v3+json",
    ...
    async def get_repository_commits(
        ...
        Args:
            ...
            owner (str): Repository owner
            repo (str): Repository name
            provider (GitProvider): Git provider (GitHub/GitLab)
            since (Optional[datetime]): Start date filter
            until (Optional[datetime]): End date filter
            branch (Optional[str]): Branch name filter
            per_page (int): Results per page
            max_pages (int): Maximum pages to fetch
        ...
        for page in range(1, max_pages + 1):
            ...
            for commit_data in response:
                try:
                    commit = await self._process_github_commit(
                        access_token, owner, repo, commit_data
                    )
                    commits.append(commit)
    ...
    async def _process_github_commit(
        self,
        access_token: str,
        owner: str,
        repo: str,
        ...
        detailed_commit = await self._make_github_request(
            access_token, f"repos/{owner}/{repo}/commits/{sha}"
        )
        ...
        return GitCommit(
            ...
            changed_files=len(detailed_commit.get("files", [])),
            url=commit_data["html_url"],
            verified=commit_info.get("verification", {}).get("verified", False)
        )
    ...
    async def get_repository_pull_requests(
        ...
        """
        Fetch pull requests/merge requests from a repository.
        ...
        Args:
            ...
            since (Optional[datetime]): Filter PRs updated since date
            per_page (int): Results per page
            max_pages (int): Maximum pages to fetch
            
        Returns:
            List[GitPullRequest]: List of pull requests
        """
        pull_requests = []
        ...
        for page in range(1, max_pages + 1):
            ...
            for pr_data in response:
                ...
                if since and updated_at < since:
                    # Since we're sorting by updated_at desc, we can break here
                    break
            ...
            for pr_data in filtered_prs:
                try:
                    pr = self._process_github_pull_request(pr_data)
                    pull_requests.append(pr)
            ...
            # If we got fewer results than requested or hit date filter, we're done
            if len(filtered_prs) < len(response) or len(response) < per_page:
                break
                
        return pull_requests

    def _process_github_pull_request(self, pr_data: Dict[str, Any]) -> GitPullRequest:
        ...
        return GitPullRequest(
            ...
            url=pr_data["html_url"],
            review_comments=pr_data.get("review_comments", 0),
            reviews_count=pr_data.get("reviews", 0)
        )
    ...
    async def get_repository_contributors(
        ...
        Returns:
            List[GitContributor]: List of contributors
        """
        contributors = []
        ...
        for contributor_data in response:
            try:
                contributor = GitContributor(
                    login=contributor_data["login"],
                    name=None,  # Would need additional API call
                    email=None,  # Not available in contributors API
                    avatar_url=contributor_data.get("avatar_url"),
                    contributions=contributor_data["contributions"],
                    commits_count=contributor_data["contributions"],
                    additions=0,  # Would need detailed analysis
                    deletions=0,  # Would need detailed analysis
                    first_contribution=None,  # Would need commit analysis
                    last_contribution=None  # Would need commit analysis
                )
                contributors.append(contributor)
    ...
    def generate_activity_heatmap(
        ...
        Args:
            commits (List[GitCommit]): Repository commits
            pull_requests (List[GitPullRequest]): Repository pull requests
            start_date (Optional[datetime]): Start date for heatmap
            end_date (Optional[datetime]): End date for heatmap
            
        Returns:
            List[ActivityHeatmapData]: Heatmap data points
        """
    ...
    async def get_comprehensive_repository_activity(
        ...
        Returns:
            GitActivityMetrics: Comprehensive activity metrics
        """
        ...
        tasks = [
            self.get_repository_commits(
                access_token, owner, repo, provider, 
        ...
        metrics = self._calculate_activity_metrics(
            commits, pull_requests, contributors, heatmap_data
        )
    ...
    def _calculate_activity_metrics(
        ...
        merged_prs = [pr for pr in pull_requests if pr.merged_at]
        avg_pr_size = (
            sum(pr.additions + pr.deletions for pr in merged_prs) / max(len(merged_prs), 1)
        )
        
        # Velocity trend (weekly aggregation)
        velocity_trend = self._calculate_velocity_trend(heatmap_data)
